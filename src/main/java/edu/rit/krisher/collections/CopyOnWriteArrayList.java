...
public class CopyOnWriteArrayList<T> implements List<T>, RandomAccess {
	...
	public CopyOnWriteArrayList(Class<T> type) {
		array = (T[]) Array.newInstance(type, 0);
	}

	public synchronized void add(int idx, T value) {
	...
	public boolean retainAll(Collection<?> arg0) {
		boolean[] retained = new boolean[size()];
		int count = 0;
		for (Object o : arg0) {
			if (useObjectEquality && o != null) {
	...
	public synchronized T[] set(T[] array) {
		final T[] oldArray = this.array;
		if (array != null) {
			final T[] newArray = (T[]) Array.newInstance(getComponentType(),
					array.length);
	...
	protected T[] addAll(int idx, Object[] array) {
		final T[] newArray = (T[]) Array.newInstance(getComponentType(),
	...
	protected synchronized T[] insert(int idx, T value) {
		final int size = size();
