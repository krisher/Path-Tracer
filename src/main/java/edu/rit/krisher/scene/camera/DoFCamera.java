...
public class DoFCamera extends PinholeCamera implements Camera {
    ...
    */
   @Override
   public void sample(final SampleRay[] raysOut, final int imageWidth, final int imageHeight, final int xOffset,
         final int yOffset, final Random rng) {
      for (final SampleRay rayOut : raysOut) {
         final double x = (xOffset + rayOut.pixelX) * 2.0 / imageWidth - 1.0;
         final double y = (yOffset + rayOut.pixelY) * 2.0 / imageHeight - 1.0;
         /*
          * X and Y sample locations will be scaled so they correspond to locations on the focal plane.
          */
         final double focalX = x * focalDist / rayZDist;
         final double focalY = y * focalDist / rayZDist;
         /*
          * Assuming the center of the lense is at 0,0,0, find the intersection of a ray from the lense center with the
          * focal plane, and transform to world coordinates.
          */
         orientation.transformVec(rayOut.direction.set(focalX, focalY, -focalDist)).add(position);

         /*
          * Jittered sample distance from the center of the lense with diameter 'aperture' (focalLength / aperture #)
          */
         ...
         /*
          * Ray origin is computed by transforming the x/y sample (assuming a 0,0,0 lense center and view looking down
          * the -z axis) to world coordinates.
          */
         orientation.transformVec(rayOut.origin.set(sampleX, sampleY, 0)).add(this.position);

         /*
          * The ray direction is a vector from the jittered origin to the convergence point
          */
   ...
   /*
    * @see edu.rit.krisher.scene.camera.PinholeCamera#lookAt(edu.rit.krisher.vecmath .Vec3, double, double, double)
    */
   @Override
